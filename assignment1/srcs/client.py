...
class Client3WH:
        ...
        if TCP in pkt:
            ...
            if (ip_src == self.dip): #Filter out outgoing packets
                ...
                if tcp_layer.flags == 'F':  
                    ...
                    finack_pkt = self.ip/TCP(sport=self.sport, dport=self.dport, flags='FA', seq=self.next_seq, ack=self.next_ack)
                    send(ack_pkt)
                    self.next_seq += 1
                ...
                if tcp_layer.flags == 'FA':  
                    self.next_ack = tcp_layer.seq + 1
                    ack_pkt = self.ip/TCP(sport=self.sport, dport=self.dport, flags='A', seq=self.next_seq, ack=self.next_ack)
                    send(ack_pkt)
                    self.next_seq += 1
    ...
    def connect(self):
        ...
        syn_pkt = self.ip/TCP(sport=self.sport, dport=self.dport, flags='S', seq=self.next_seq)
        self.next_seq += 1  
        send(syn_pkt)
    ...
    def close(self):
        ...
        fin_pkt = self.ip/TCP(sport=self.sport, dport=self.dport, flags='FA', seq=self.next_seq, ack=self.next_ack)
        send(fin_pkt)
    ...
    def send(self, payload):
        ...
        data_pkt = self.ip/TCP(sport=self.sport, dport=self.dport, flags="PA", seq=self.next_seq, ack=self.next_ack)/Raw(load=payload)
